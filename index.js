...
let demandesWeb = {};
try { demandesWeb = require('./demandes-web'); console.log('✅ Module demandes web chargé'); }
catch (e) { console.log('⚠️ demandes-web non chargé:', e.message); }
...
async function routeTresorerieInteraction(interaction) {
  try {
    ...
    // (Le panneau « coffre-entreprise » que j'avais ajouté est retiré : #coffre-entreprise a déjà
    //  le panneau complet « 💰 Trésorerie ». On ne garde donc plus les boutons centr_*.)
...
// #coffre-entreprise a déjà le panneau complet « 💰 Trésorerie » (notion-modules-v2).
// Le panneau « Coffre commun » que j'avais ajouté était un DOUBLON → on le retire (une fois),
// pour ne garder qu'un seul panneau dans ce salon.
async function _retirerPanelCoffreEntrepriseDouble(guild) {
  try {
    ...
    const me = guild.client.user.id;
    const db = loadDB();
    let msg = db.coffreEntreprisePanelId ? await ch.messages.fetch(db.coffreEntreprisePanelId).catch(() => null) : null;
    if (!msg) { const msgs = await ch.messages.fetch({ limit: 30 }).catch(() => null); msg = msgs ? [...msgs.values()].find(m => { const t = m.embeds?.[0]?.title || ''; return m.author?.id === me && t.includes('Coffre commun') && t.includes('Iron Wolf'); }) : null; }
    if (msg) { await msg.delete().catch(() => {}); }
    if (db.coffreEntreprisePanelId) { const d = loadDB(); delete d.coffreEntreprisePanelId; saveDB(d); }
  } catch (e) { console.log('⚠️ retrait panneau coffre-entreprise (doublon):', e.message); }
}
...
async function autoSetup(guild) {
  ...
  _retirerPanelCoffreEntrepriseDouble(guild).catch(() => {}); // retire mon ancien panneau en double (le vrai « 💰 Trésorerie » reste)
  ...
  medical.installerDossiers?.(guild).then((n) => console.log(`🩺 Dossiers vivants garantis (${n || 0} nouveau(x) fil(s))`)).catch(() => {});
...
client.once('clientReady', async () => {
  ...
  // 📨 Nouvelles demandes du site (guichet unique) → notification Discord + lien profond (toutes les 2 min).
