    ...
    for (const [id, membre] of Object.entries(db.members || {})) {
      ...
      try {
        ...
        await member.send({ embeds: [new EmbedBuilder()
          .setColor(0xED4245).setTitle('🚪 Iron Wolf Company')
        ...
        if (!kicked_ok) { console.log(`⚠️ Impossible de kicker ${member.user.username}`); continue; }
        delete db.members[id];
        kicked++;
        if (logsCh) await logsCh.send({ embeds: [new EmbedBuilder()
          .setColor(0xED4245).setTitle(`🚪 AUTO-KICK — ${member.user.username}`)
          .addFields(
...
async function envoyerRapportDirection(guild) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '👥 MEMBRES', value: [`🆕 Arrivées : **${nouveaux.length}**`, `🚪 Départs : **${departs.length}**`, `👁️ Visiteurs en attente : **${visiteurs.length}**`, alertes.length > 0 ? `⚠️ Inactifs : **${alertes.length}** — ${alertes.slice(0,3).map(m => m.name).join(', ')}${alertes.length > 3 ? '...' : ''}` : '✅ Aucune inactivité'].join('\n'), inline: false },
        ...
        { name: '🎯 OPÉRATIONS', value: [opsEnCours.length > 0 ? `🟢 En cours : **${opsEnCours.length}** — ${opsEnCours.map(o => o.name).join(', ')}` : '🟢 Aucune en cours', opsTermHier.length > 0 ? `✅ Terminées hier : **${opsTermHier.length}** — ${opsTermHier.map(o => `${o.name} (${o.resultat || '—'})`).join(', ')}` : ''].filter(Boolean).join('\n') || '*Aucune activité opérationnelle*', inline: false },
    ...
    console.log(`✅ Rapport quotidien envoyé à ${envoyes} membre(s) Direction`);
...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'stats') {
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x8B1A1A).setTitle('📊 Statistiques — Iron Wolf Company')
      .addFields(
        { name: '👥 Membres',     value: [`✅ Actifs : **${members.filter(m => m.status === 'actif').length}**`, `⚠️ Absents : **${members.filter(m => m.status === 'absent').length}**`, `👁️ Total : **${members.filter(m => m.status !== 'parti').length}**`].join('\n'), inline: true },
        { name: '🎯 Opérations',  value: [`🟢 En cours : **${ops.filter(o => o.status === 'en_cours').length}**`, `🟡 Prépa : **${ops.filter(o => o.status === 'preparation').length}**`, `✅ Terminées : **${ops.filter(o => o.status === 'terminee').length}**`].join('\n'), inline: true },
        { name: '📋 Recrutement', value: [`📥 En attente : **${cands.filter(c => c.status === 'reçue').length}**`, `✅ Acceptés : **${cands.filter(c => c.status === 'acceptee').length}**`].join('\n'), inline: true },
        { name: '📜 Contrats',    value: [`✅ Signés : **${contrats.filter(c => c.status === 'signe').length}**`, `🟡 En attente : **${contrats.filter(c => c.status === 'en_attente').length}**`].join('\n'), inline: true },
        { name: '💰 Trésorerie',  value: [`⚖️ Légal : **$${(db.coffres?.legal || 0).toLocaleString('fr-FR')}**`, `🔒 Illégal : **$${(db.coffres?.illegal || 0).toLocaleString('fr-FR')}**`].join('\n'), inline: true },
  ...
  if (commandName === 'solde') {
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('💰 Soldes des coffres — IWC')
      .addFields(
        { name: '⚖️ Coffre Légal',   value: `**$${soldeLegal.toLocaleString('fr-FR')}**`,  inline: true },
        { name: '🔒 Coffre Illégal', value: `**$${soldeIlleg.toLocaleString('fr-FR')}**`,  inline: true },
        { name: '💎 Total',          value: `**$${(soldeLegal + soldeIlleg).toLocaleString('fr-FR')}**`, inline: true },
      ).setFooter({ text: `IWC • ${fmtShort(new Date())}` })
    ], flags: isDirection(interaction.member) ? undefined : MessageFlags.Ephemeral });
    return;
  }
  ...
  if (commandName === 'fiche') {
    ...
    if (!cand) {
      ...
      if (nomIC) {
        const discordId = MEMBRES_DISCORD_MAP[nomIC]; const membre = db.members[discordId];
        await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x8B1A1A).setTitle(`👤 Fiche — ${nomIC}`).setDescription('*Membre fondateur — fiche à compléter dans Notion*')
          .addFields({ name: '🎭 Personnage', value: nomIC, inline: true }, { name: '📋 Statut', value: membre?.status === 'absent' ? '⚠️ Absent' : '✅ Actif', inline: true }, { name: '🎖️ Rang', value: membre?.rang || '—', inline: true })
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(cand.type === 'illegal' ? 0x8B1A1A : 0x3B82F6).setTitle(`👤 Fiche — ${cand.nomPerso}`)
      .addFields(
        { name: '🎭 Personnage', value: cand.nomPerso, inline: true }, { name: '🎂 Âge', value: cand.agePerso || '—', inline: true },
        { name: '⚖️ Pôle', value: cand.type === 'illegal' ? '🔪 Illégal' : '⚖️ Légal', inline: true },
        { name: '💼 Métier', value: cand.metier || cand.specialite || '—', inline: true },
        { name: '🕐 Disponibilités', value: cand.dispos || '—', inline: true },
        { name: '📅 Entrée', value: fmtShort(cand.acceptedAt), inline: true },
  ...
  if (commandName === 'ops') {
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFFA500).setTitle('🎯 Opérations actives — IWC')
      .setDescription(opsActives.map(o => [`**${o.name}** — ${o.status === 'en_cours' ? '🟢 En cours' : '🟡 Préparation'}`, `📍 ${o.lieu || '—'} · 👥 ${(o.participants || []).join(', ') || 'Aucun participant'}`].join('\n')).join('\n\n'))
      .setFooter({ text: `IWC • ${fmtShort(new Date())}` })
    ], ephemeral: false });
    return;
  }
  ...
  if (commandName === 'absent') {
    ...
    if (absCh) await absCh.send({ embeds: [new EmbedBuilder().setColor(0xFFA500).setTitle(`🟡 Absence — ${interaction.user.username}`)
      .addFields({ name: '👤 Membre', value: `<@${interaction.user.id}>`, inline: true }, { name: '⏱️ Durée', value: duree, inline: true }, { name: '📝 Raison', value: raison, inline: true })
      .setFooter({ text: `IWC • ${fmtShort(new Date())}` })
    ] });
    return;
  }
  ...
  if (commandName === 'promo') {
    ...
    await membre.send({ embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('⬆️ Promotion — Iron Wolf Company')
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setTitle(`⬆️ Promotion — ${cible.username}`)
      .addFields({ name: '👤 Membre', value: `<@${cible.id}>`, inline: true }, { name: '📉 Ancien rang', value: ancienRang, inline: true }, { name: '📈 Nouveau rang', value: nouveauRang, inline: true }, { name: '✅ Décidé par', value: interaction.user.username, inline: true })
      .setFooter({ text: `IWC • ${fmtShort(new Date())}` })
    ] });
    return;
  }
  ...
  if (commandName === 'retro') {
    ...
    await membre.send({ embeds: [new EmbedBuilder().setColor(0xED4245).setTitle('⬇️ Rétrogradation — Iron Wolf Company')
    ...
    await interaction.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setTitle(`⬇️ Rétrogradation — ${cible.username}`)
      .addFields({ name: '👤 Membre', value: `<@${cible.id}>`, inline: true }, { name: '📉 Ancien rang', value: ancienRang, inline: true }, { name: '📈 Nouveau rang', value: nouveauRang, inline: true }, { name: '📝 Raison', value: raison, inline: true }, { name: '✅ Décidé par', value: interaction.user.username, inline: true })
      .setFooter({ text: `IWC • ${fmtShort(new Date())}` })
    ] });
    return;
  }
...
// ── Nettoyage des messages épinglés / doublons du bot ──
async function cleanBotPinnedMessages(guild, ...channelNames) {
  for (const name of channelNames) {
    try {
      const ch = getCh(guild, name);
      if (!ch) continue;

      // Dépingler et supprimer les messages épinglés du bot
      const pinned = await ch.messages.fetchPinned().catch(() => null);
      if (pinned) {
        for (const [, msg] of pinned) {
          if (msg.author.id !== guild.members.me?.id) continue;
          await msg.unpin().catch(() => {});
          await msg.delete().catch(() => {});
          console.log(`🧹 Message épinglé supprimé dans #${ch.name}`);
        }
      }

      // Supprimer les doublons (garder uniquement le plus récent)
      ...
      for (let i = 1; i < botMsgs.length; i++) {
        ...
        console.log(`🧹 Doublon supprimé dans #${ch.name}`);
      }
...
async function autoSetup(guild) {
  ...
  // Nettoyage des anciens messages épinglés et doublons
  await cleanBotPinnedMessages(guild,
  ...
  if (coffreIlleg) {
    const msgs = await coffreIlleg.messages.fetch({ limit: 10 });
    if (!msgs.find(m => m.author.id === client.user.id && m.content.includes('COFFRE')))
      await coffreIlleg.send('```\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n🔒 COFFRE — FINANCES ILLÉGALES\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n```\n*Ce canal est strictement confidentiel.*\n\n**Format :**\n```\nTYPE      : Entrée / Sortie\nMONTANT   : $000\nOBJET     : \nRESPONSABLE: \nSOLDE     : $000\n```');
  }
...
async function notionQueryAgenda() {
  ...
  try {
    ...
    return (data.results || []).map(p => ({
      id: p.id, titre: p.properties.Titre?.title?.[0]?.plain_text || '—', date: p.properties.Date?.date?.start,
      heure: p.properties.Heure?.rich_text?.[0]?.plain_text, type: p.properties.Type?.select?.name || 'Réunion',
      participants: p.properties.Participants?.multi_select?.map(x => x.name) || [],
      lieu: p.properties.Lieu?.rich_text?.[0]?.plain_text || '—', notes: p.properties.Notes?.rich_text?.[0]?.plain_text,
      notif24: p.properties['Notif 24h']?.checkbox, notif1h: p.properties['Notif 1h']?.checkbox,
      notif15: p.properties['Notif 15min']?.checkbox, statut: p.properties['Statut']?.select?.name || '', url: p.url,
    }));
  } catch { return []; }
}
...
function buildParticipantMentions(participants) {
  return participants.map(name => { const val = PARTICIPANTS_MAP[name]; if (!val) return null; return val.startsWith('<@') ? val : `<@${val}>`; }).filter(Boolean).join(' ');
}
  ...
  for (const name of (appt.participants || [])) {
    ...
    try {
      ...
      await member.send({ embeds: [new EmbedBuilder().setColor(color).setTitle(title)
        ...
        .addFields({ name: '🗓️ Quand', value: `${fmtLong(appt.date)}${appt.heure ? ` à **${appt.heure}**` : ''}`, inline: true }, { name: '📍 Lieu', value: appt.lieu, inline: true }, ...(appt.notes ? [{ name: '📝 Notes', value: appt.notes }] : []), { name: '🔗 Notion', value: `[Ouvrir](${appt.url})` })
        .setFooter({ text: 'IWC • Rappel automatique' })
      ] });
    } catch {}
  }
...
async function checkAgenda(guild) {
  ...
  for (const a of appts) {
    ...
    const mkEmbed = (t, c) => new EmbedBuilder().setColor(c).setTitle(t)
      ...
      .addFields({ name: 'Quand', value: `${fmtLong(a.date)}${heureAff ? ` à **${heureAff}**` : ''}`, inline: true }, { name: 'Lieu', value: a.lieu, inline: true }, { name: 'Participants', value: a.participants.length > 0 ? a.participants.join(', ') : '—' }, ...(a.notes ? [{ name: 'Notes', value: a.notes }] : []), { name: 'Modifier', value: `[Notion](${a.url})` })
...
client.on('messageCreate', async message => {
  ...
  if (opsCh && message.channel.id === opsCh.id && isDirection(message.member)) {
    if (message.content.toUpperCase().includes('OPÉRATION') || message.content.toUpperCase().includes('OPERATION')) {
      ...
      const embed = new EmbedBuilder().setColor(0xFFA500).setTitle(`🎯 OPÉRATION — ${op.name}`)
        .addFields({ name: 'Statut', value: '🟡 En préparation', inline: true }, { name: 'Pôle', value: pole === 'legal' ? '⚖️ Pôle Légal' : '🔪 Pôle Illégal', inline: true }, { name: 'Lieu', value: op.lieu, inline: true }, { name: 'Objectif', value: op.objectif }, { name: 'Équipe', value: op.equipe }, { name: '👥 Participants (0)', value: '*Personne pour l\'instant. Clique « ✋ Je participe » ci-dessous.*' })
  ...
  // ── #planning — image → Notion (plus de panneau, plus de SESSION) ──
  ...
  if (planCh && message.channel.id === planCh.id) {
    ...
    // Texte seul → ignoré silencieusement
    return;
  }

  // ── #plans — archive photos de lieux ──
...
client.on('interactionCreate', async interaction => {
  ...
  // ── MODAUX v2 — trésorerie (customId commence par modal_tresor_) ──
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === 'btn_hierarchie_refresh') {
      ...
      return interaction.editReply({ content: '✅ Hiérarchie mise à jour.' });
    }
  ...
  if (interaction.isModalSubmit() && interaction.customId.startsWith('op_resultat_modal_')) {
    ...
    op.status = 'terminee'; op.endedAt = new Date().toISOString();
    ...
    await notionModules.ajouterJournalIC?.(guild, { type: 'operation', emoji: '✅', titre: `Opération terminée — ${op.name}`, description: `Résultat : **${op.resultat}** · Butin : ${op.butin} · Participants : ${(op.participants || []).join(', ') || '—'}`, auteur: interaction.user.username });
  ...
  if (interaction.isButton() && interaction.customId.startsWith('signer_offre_')) {
    ...
    await notionModules.ajouterJournalIC?.(guild, { type: 'contrat', emoji: '📜', titre: `Contrat signé — ${contratId}`, description: `Client : **${contrat.clientNom}** · Mission : ${contrat.objet} · Rémunération : ${contrat.remuneration}`, auteur: interaction.user.username });
  ...
  if (interaction.isButton() && interaction.customId.startsWith('signer_emploi_')) {
    ...
    await notionModules.ajouterJournalIC?.(guild, { type: 'contrat', emoji: '📥', titre: `Contrat employeur signé — ${contratId}`, description: `Employeur : **${contrat.employeurNom}** · Mission : ${contrat.objet} · Signé par ${interaction.user.username}`, auteur: interaction.user.username });
    await interaction.update({ embeds: [EmbedBuilder.from(interaction.message.embeds[0]).setColor(0x57F287).spliceFields(5, 1, { name: '📌 Statut', value: `✅ Signé le ${fmtShort(new Date())} par ${interaction.user.username}`, inline: true })], components: [] });
