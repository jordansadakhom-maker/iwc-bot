// ═══════════════════════════════════════════════════════════════
// poker-image.js — Rend la table de POKER 5-CARD DRAW en VRAIE image
// (sharp + SVG) : tapis vert feutre, cartes dessinées (enseignes
// vectorielles, jamais dépendantes d'une police), DOS de carte
// ornementé (emblème Iron Wolf « IW ») pour les mains face cachée du
// plateau public, et un rendu de main privée (face visible) pour le
// coup d'œil éphémère (pk_peek).  sharp est chargé PARESSEUSEMENT :
// s'il manque, on renvoie null et poker.js retombe sur l'affichage
// texte (jamais de crash).
// ═══════════════════════════════════════════════════════════════
let _sharp = null, _tried = false;
function getSharp() { if (_tried) return _sharp; _tried = true; try { _sharp = require('sharp'); } catch (e) { console.log('⚠️ poker-image: sharp indisponible →', e.message); _sharp = null; } return _sharp; }
...
function ligneCartes(cards, x, y, cw, ch, gap, reveal) {
  let out = '';
  cards.forEach((c, i) => {
    const cx = x + i * (cw + gap);
    out += reveal ? carteFace(cx, y, cw, ch, c.r, c.s) : carteDos(cx, y, cw, ch);
  });
  return out;
}

// ─── Plateau public ───
// state = { sousTitre, pot, joueurs:[{nom, ante, cards:[{r,s}], reveal, actif, badge}] }
async function genererTable(state) {
  const sharp = getSharp();
  if (!sharp) return null;
  try {
    const W = 980;
    const CW = 60, CH = 86, GAP = 8;            // 5 cartes par joueur
    const headH = 84, rowH = 108;
    const joueurs = state.joueurs || [];
    const H = headH + Math.max(1, joueurs.length) * rowH + 30;

    const S = [];
    S.push(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`);
    // Tapis (dégradé feutre identique au blackjack)
    S.push(`<defs><radialGradient id="felt" cx="50%" cy="30%" r="80%"><stop offset="0%" stop-color="#12704a"/><stop offset="100%" stop-color="#093a25"/></radialGradient></defs>`);
    ...
    // En-tête
    S.push(`<text x="44" y="50" font-family="DejaVu Serif, Georgia, serif" font-size="27" font-weight="bold" fill="#e7cf94" letter-spacing="2">SALOON · POKER · 5-CARD DRAW</text>`);
    if (state.sousTitre) S.push(`<text x="44" y="72" font-family="DejaVu Sans, Arial, sans-serif" font-size="15" fill="#d8e8dd">${esc(state.sousTitre)}</text>`);
    // Pot (pastille or à droite)
    if (state.pot != null) {
      S.push(`<rect x="${W - 250}" y="30" width="220" height="40" rx="20" fill="#0c3f28" stroke="#e7cf94" stroke-width="1.6"/>`);
      S.push(`<text x="${W - 140}" y="56" font-family="DejaVu Sans, Arial, sans-serif" font-size="18" font-weight="bold" fill="#ffe8a3" text-anchor="middle">POT : ${esc(String(state.pot))}</text>`);
    }
    ...
    joueurs.forEach((j, idx) => {
      ...
      S.push(`<text x="44" y="${y + 48}" font-family="DejaVu Sans, Arial, sans-serif" font-size="14" fill="#bcd7c6">Ante ${esc(String(j.ante))}</text>`);
      if (j.badge) S.push(`<text x="44" y="${y + 70}" font-family="DejaVu Sans, Arial, sans-serif" font-size="15" font-weight="bold" fill="${j.gagnant ? '#ffe08a' : '#cfe3d6'}">${esc(j.badge)}</text>`);
      const cardsX = 300;
      S.push(ligneCartes(j.cards || [], cardsX, y + 4, CW, CH, GAP, !!j.reveal));
    });
    if (!joueurs.length) S.push(`<text x="${W / 2}" y="${headH + 60}" font-family="DejaVu Sans, Arial, sans-serif" font-size="17" fill="#cfe3d6" text-anchor="middle" font-style="italic">Aucun joueur à la table — asseyez-vous pour jouer.</text>`);

    S.push('</svg>');
    return await sharp(Buffer.from(S.join(''))).png().toBuffer();
  } catch (e) { console.log('⚠️ poker-image genererTable:', e.message); return null; }
}

// ─── Main privée (coup d'œil éphémère pk_peek) ───
// state = { nom, cards:[{r,s}], evalNom, sousTitre }
async function genererMain(state) {
  const sharp = getSharp();
  if (!sharp) return null;
  try {
    const cards = state.cards || [];
    const CW = 96, CH = 138, GAP = 14;
    const marge = 40, headH = 90;
    const W = Math.max(560, marge * 2 + cards.length * CW + (cards.length - 1) * GAP);
    const H = headH + CH + 90;

    const S = [];
    S.push(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`);
    S.push(`<defs><radialGradient id="felt2" cx="50%" cy="35%" r="80%"><stop offset="0%" stop-color="#12704a"/><stop offset="100%" stop-color="#093a25"/></radialGradient></defs>`);
    S.push(`<rect width="${W}" height="${H}" fill="#0a2a1c"/>`);
    S.push(`<rect x="10" y="10" width="${W - 20}" height="${H - 20}" rx="22" fill="url(#felt2)" stroke="#c8a45c" stroke-width="3"/>`);
    S.push(`<text x="${W / 2}" y="50" font-family="DejaVu Serif, Georgia, serif" font-size="24" font-weight="bold" fill="#e7cf94" text-anchor="middle" letter-spacing="2">VOTRE MAIN — ${esc(state.nom || '')}</text>`);
    if (state.sousTitre) S.push(`<text x="${W / 2}" y="76" font-family="DejaVu Sans, Arial, sans-serif" font-size="14" fill="#cfe3d6" text-anchor="middle">${esc(state.sousTitre)}</text>`);
    const startX = (W - (cards.length * CW + (cards.length - 1) * GAP)) / 2;
    S.push(ligneCartes(cards, startX, headH, CW, CH, GAP, true));
    if (state.evalNom) S.push(`<text x="${W / 2}" y="${headH + CH + 46}" font-family="DejaVu Sans, Arial, sans-serif" font-size="22" font-weight="bold" fill="#ffe8a3" text-anchor="middle">${esc(state.evalNom)}</text>`);
    S.push('</svg>');
    return await sharp(Buffer.from(S.join(''))).png().toBuffer();
  } catch (e) { console.log('⚠️ poker-image genererMain:', e.message); return null; }
}

module.exports = { genererTable, genererMain };
