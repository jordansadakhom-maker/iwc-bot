...
export default async function InventairePage() {
  const { connecte, vehicules } = await getInventaire();

  return (
    <>
      <PageHeader titre="Inventaire" sous="Matériel &amp; véhicules des deux pôles" actif={connecte} />
      <Card>
        <CardHeader titre="Véhicules" compteur={vehicules.length} />
        {vehicules.length === 0 ? (
          <Empty icon={Boxes}>
            Aucun véhicule enregistré. L&apos;armement (registre d&apos;armes) et le coffre matériel de ton Discord pourront aussi être branchés ici — dis-le moi et je les ajoute.
          </Empty>
        ) : (
