...
export async function getNotifications(): Promise<{ items: Notif[]; count: number }> {
  ...
  if (habilite) for (const f of factures || []) {
    ...
    if (Number.isFinite(emis) && nowMs - emis > 7 * 86400000) items.push({ id: "fa7-" + f.objet, severite: "alerte", type: "Facture", texte: `Impayé depuis plus de 7 jours${relanceTxt} : ${f.objet} (${num(f.montant)}$)`, href: "/dispensaire/factures", ref: String(f.objet), escalade });
    else if (t < nowMs) items.push({ id: "fa-" + f.objet, severite: "alerte", type: "Facture", texte: `Délai de paiement dépassé${relanceTxt} : ${f.objet} (${num(f.montant)}$)`, href: "/dispensaire/factures", ref: String(f.objet), escalade });
