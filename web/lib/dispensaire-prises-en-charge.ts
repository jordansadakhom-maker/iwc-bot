...
export async function getPrisesEnCharge(): Promise<PrisesEnChargeData> {
  const vide: PrisesEnChargeData = { pret: false, canEdit: false, canSoigner: false, enCours: [], recentes: [], patients: [], medecins: [] };
  const admin = createAdminClient();
  if (!admin) return vide;
  const [canEdit, canSoigner] = await Promise.all([estAutorise(), peutSoigner()]);
  ...
  if (error) return { ...vide, pret: false, canEdit, canSoigner };   // table absente (SQL non lancé)
  ...
  return { pret: true, canEdit, canSoigner, enCours, recentes, patients, medecins };
}
