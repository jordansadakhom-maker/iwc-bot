...
export type RhData = { connecte: boolean; pret: boolean; canEdit: boolean; salaries: Salarie[] };
...
export async function getRh(): Promise<RhData> {
  const vide: RhData = { connecte: false, pret: false, canEdit: false, salaries: [] };
  ...
  if (error) return { connecte: true, pret: false, canEdit, salaries: [] };
  ...
  return { connecte: true, pret: true, canEdit, salaries };
}
