...
export function DispensaireAccueilCockpit({ d, rdvAujourdhui, ordre = [], masques = [], onSave }: { d: AccueilData; rdvAujourdhui: number; ordre?: string[]; masques?: string[]; onSave?: SaveFn }) {
  ...
  const widgets = useMemo<Widget[]>(() => [
    ...
    { key: "matieres", label: "Matières en rupture", node: <StatWidget label="Matières en rupture" value={matRupture} icon={FlaskConical} tone={matRupture ? "crit" : "steel"} href="/dispensaire/stockage" sous={matRupture ? d.matieresRupture.slice(0, 2).map((m) => m.nom).join(" · ") : "Rien à commander"} /> },
