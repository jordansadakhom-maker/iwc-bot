...
import { FDO_PRIX, money, type FDOData, type SoinFDO } from "@/lib/dispensaire-facturation-const";
import { Flash, inputCls } from "@/components/edit-ui";
import { creerSoin, supprimerSoin } from "@/app/dispensaire/fdo/actions";
...
export function DispensaireFDO({ data }: { data: FDOData }) {
  ...
  async function ajouter() {
    ...
    cleRef.current = "";
    const tmp: SoinFDO = { id: r.id || "tmp", bureau: v.bureau.trim(), agent: v.agent || null, soin: null, montant: FDO_PRIX, statut: "facture", note: null, par: null, createdAt: new Date().toISOString() };
    setSoins((p) => [tmp, ...p]);
    setV({ bureau: v.bureau, agent: "" });
    ...
    <div className="flex flex-col gap-4">
      ...
      <section className="rounded-[14px] border border-border bg-surface p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <label className="flex flex-col gap-1"><span className="text-[0.7rem] uppercase tracking-[0.05em] text-faint">Nom du shérif</span><input className={inputCls} value={v.agent} onChange={set("agent")} placeholder="Nom du shérif / agent" /></label>
              ...
              return (
                <div key={s.id} className="group flex items-center gap-2 rounded-[10px] border border-border bg-surface-2 px-3 py-2 text-[0.8rem]">
                  <div className="min-w-0 flex-1">
                    <div className="truncate"><b className="font-semibold">{s.agent || "Agent"}</b></div>
