...
const EQUIPEMENTS = [
  "Fusils", "Revolvers", "Munitions", "Trousse médicale", "Vivres", "Eau", "Lanterne",
...
const ETAPES = [
  ...
  { n: 4, label: "Rémunération", sous: "Primes & butin" },
...
function CarteForm({ titre, icon: Icon, children }: { titre: string; icon: typeof Shield; children: React.ReactNode }) {
  return (
...
function SceauLoup({ size = 60 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden="true" style={{ transform: "rotate(-9deg)", filter: "drop-shadow(0 3px 4px rgba(60,20,12,.4))" }}>
      ...
      {/* museau de loup stylisé, embossé */}
...
// Carte « à l'ancienne » stylisée (tracé à l'encre) — pas un GPS, pur décor 1904.
...
function DossierVivant({ d, dateStr, auteur }: { d: Dossier; dateStr: string; auteur: string }) {
  ...
  return (
    <div id="op-dossier-doc" style={{ position: "relative", fontFamily: "Georgia,'Times New Roman',serif", color: "#2c2013", background: "linear-gradient(#f6ecd3,#efe3c6)", border: "1px solid #b39c6e", borderRadius: 10, padding: "26px 26px 30px", boxShadow: "inset 0 0 0 4px #f8f2e2, inset 0 0 0 5px #c2a86f, 0 10px 30px rgba(40,26,10,.28)" }}>
      ...
      {(d.objectif || d.description) ? (<><SectionDoc titre="OBJECTIF" />
        {d.objectif ? <div style={{ fontSize: 12.5, fontWeight: 600 }}>{d.objectif}</div> : null}
        {d.description ? <div style={{ fontSize: 11.5, lineHeight: 1.5, marginTop: 4, textAlign: "justify", color: "#3a2c17" }}>{d.description}</div> : null}</>) : null}
...
export function OperationWizard({ membres, pole, auteur }: { membres: MembreLite[]; pole: string; auteur: string }) {
  ...
  const [d, setD] = useState<Dossier>(() => ({ ...VIDE, codeOp: genCode(), pole: pole === "illegal" ? "illegal" : "legal" }));
  ...
  // Restaure un brouillon éventuel.
  useEffect(() => {
    try { const raw = localStorage.getItem(DRAFT_KEY); if (raw) { const j = JSON.parse(raw); if (j && typeof j === "object") setD((p) => ({ ...p, ...j })); } } catch { /* ignore */ }
    chargé.current = true;
  }, []);
  ...
  const alertes = useMemo(() => {
    ...
    const m = menaceInfo(d.menace); const nbAgents = Number(d.agents) || 0; const nbVeh = Number(d.vehicules) || 0;
    const prime = Number(String(d.primeGlobale).replace(/[^\d.-]/g, "")) || 0;
  ...
  async function creer() {
    ...
    const r = await creerOperation({
      ...
      prime: d.primeGlobale || undefined, lieu: lieu || undefined, objectif: d.objectif || d.description || undefined,
  ...
  return (
    <>
      <style>{`@media print{body *{visibility:hidden!important}#op-dossier-doc,#op-dossier-doc *{visibility:visible!important}#op-dossier-doc{position:fixed!important;inset:0!important;margin:0!important;border-radius:0!important;box-shadow:none!important;max-width:none!important}}`}</style>
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[0.72rem]">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 font-semibold" style={{ borderColor: "color-mix(in srgb,var(--good) 40%,var(--border))", color: "var(--good)" }}><span className="disp-live-dot h-1.5 w-1.5 rounded-full" style={{ background: "var(--good)" }} /> Brouillon en direct</span>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(340px,420px)]">
        ...
        <div className="min-w-0">
              ...
              {ETAPES.map((e, i) => {
                ...
                return (
                  <div key={e.n} className="flex items-center">
                    <button onClick={() => setStep(e.n)} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-left transition" style={{ background: on ? "color-mix(in srgb,var(--accent) 12%,transparent)" : "transparent" }}>
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full text-[0.72rem] font-bold" style={{ background: on ? "var(--accent)" : done ? "color-mix(in srgb,var(--good) 25%,transparent)" : "var(--surface-2)", color: on ? "#1a1206" : done ? "var(--good)" : "var(--faint)", border: `1px solid ${on ? "var(--accent)" : "var(--border)"}` }}>{done ? <Check className="h-3.5 w-3.5" /> : e.n}</span>
                      <span className="hidden sm:block">
                        <span className="block text-[0.76rem] font-semibold leading-none" style={{ color: on ? "var(--ink)" : "var(--muted)" }}>{e.label}</span>
                        <span className="block text-[0.62rem] text-faint">{e.sous}</span>
                      </span>
                    </button>
                    {i < ETAPES.length - 1 ? <ChevronRight className="h-3.5 w-3.5 shrink-0 text-faint" /> : null}
                  </div>
                );
              })}
            </div>
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-4">
            <button onClick={sauverBrouillon} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3 py-2 text-[0.8rem] font-semibold hover:border-border-2"><Save className="h-3.5 w-3.5" /> Enregistrer le brouillon</button>
            <div className="flex items-center gap-2">
              ...
              {step < 5 ? (
                <button onClick={() => setStep((s) => s + 1)} className="inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-[0.82rem] font-semibold text-black/85" style={{ background: "var(--accent)" }}>Continuer → {ETAPES[step].label} <ChevronRight className="h-3.5 w-3.5" /></button>
              ) : (
        ...
        <div className="min-w-0">
          <div className="sticky top-4">
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="text-[0.7rem] font-semibold uppercase tracking-[0.1em] text-faint">Aperçu du dossier</span>
              <button onClick={imprimer} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-2.5 py-1.5 text-[0.72rem] font-semibold hover:border-border-2"><Printer className="h-3.5 w-3.5" /> Vue imprimable</button>
            </div>
  ...
  function renderStep() {
      ...
      <>
        <CarteForm titre="Informations générales" icon={FileText}>
          <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
            <Field label="Titre / Objectif de la mission" req><input className={inputCls} value={d.titre} onChange={(e) => set("titre", e.target.value.slice(0, 120))} placeholder="Escorte d'un convoi vers Saint-Denis" /></Field>
            <Field label="Code opération"><input className={inputCls + " w-full sm:w-32 font-num"} value={d.codeOp} onChange={(e) => set("codeOp", e.target.value.slice(0, 20))} /></Field>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Nom radio" hint="Nom de code transmis par télégraphe"><div className="flex items-center gap-1.5"><Radio className="h-4 w-4 text-faint" /><input className={inputCls} value={d.nomRadio} onChange={(e) => set("nomRadio", e.target.value.slice(0, 40))} placeholder="BLACK WOLF" /></div></Field>
          ...
          <Field label="Description complète & contexte"><textarea className={inputCls + " min-h-[110px] resize-y"} value={d.description} onChange={(e) => set("description", e.target.value.slice(0, 1000))} placeholder="Pourquoi cette mission existe, ce qui est attendu, les menaces connues…" /><Compteur v={d.description} max={1000} /></Field>
          <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Type d'opération">
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
              {TYPES.map((t) => { const on = d.type === t.key; return (
                <button key={t.key} onClick={() => set("type", t.key)} className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[0.78rem] font-semibold transition" style={{ borderColor: on ? "var(--accent)" : "var(--border)", background: on ? "color-mix(in srgb,var(--accent) 12%,transparent)" : "var(--surface-2)", color: on ? "var(--accent)" : "var(--muted)" }}><t.icon className="h-3.5 w-3.5" /> {t.label}</button>
              ); })}
            </div>
          </Field>
          <Field label="Niveau de menace">
            <div className="flex flex-wrap gap-1.5">
              {MENACES.map((m) => { const on = d.menace === m.key; return (
                <button key={m.key} onClick={() => set("menace", m.key)} className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[0.78rem] font-semibold transition" style={{ borderColor: on ? m.tone : "var(--border)", background: on ? `color-mix(in srgb,${m.tone} 16%,transparent)` : "var(--surface-2)", color: on ? m.tone : "var(--muted)" }}>{m.label}</button>
              ); })}
            </div>
            <div className="mt-2 flex items-center gap-2 text-[0.72rem] text-faint"><span>Préparation conseillée :</span> <b style={{ color: menaceInfo(d.menace).tone }}>{menaceInfo(d.menace).agents} agent(s) minimum</b></div>
          </Field>
        </CarteForm>
      </>
    );
    ...
    if (step === 2) return (
      <>
        <CarteForm titre="Équipe" icon={Users}>
          <Field label="Pôle responsable"><Puces options={[{ key: "legal", label: "Iron Wolf" }, { key: "illegal", label: "La Confrérie" }]} value={d.pole} onChange={(v) => set("pole", v)} /></Field>
          <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <Field label="Nombre d'agents"><input className={inputCls} type="number" min={0} value={d.agents} onChange={(e) => set("agents", e.target.value)} placeholder="5" /></Field>
            <Field label="Invités / mercenaires"><input className={inputCls} type="number" min={0} value={d.invites} onChange={(e) => set("invites", e.target.value)} placeholder="0" /></Field>
            <Field label="Véhicules"><input className={inputCls} type="number" min={0} value={d.vehicules} onChange={(e) => set("vehicules", e.target.value)} placeholder="2" /></Field>
          </div>
        ...
        <CarteForm titre="Matériel" icon={Wrench}>
          <div className="flex flex-wrap gap-1.5">
            {[...EQUIPEMENTS, ...d.equipement.filter((e) => !EQUIPEMENTS.includes(e))].map((e) => { const on = d.equipement.includes(e); return (
              <button key={e} onClick={() => toggleEquip(e)} className="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[0.76rem] font-semibold transition" style={{ borderColor: on ? "var(--brass)" : "var(--border)", background: on ? "color-mix(in srgb,var(--brass) 14%,transparent)" : "var(--surface-2)", color: on ? "var(--brass)" : "var(--muted)" }}>{on ? <Check className="h-3 w-3" /> : null} {e}</button>
            ); })}
          </div>
          <div className="flex items-center gap-1.5">
            <input className={inputCls} value={equipCustom} onChange={(e) => setEquipCustom(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addEquipCustom(); } }} placeholder="Ajouter un matériel…" maxLength={40} />
            <button onClick={addEquipCustom} className="shrink-0 rounded-lg border border-border bg-surface-2 px-3 py-2 text-[0.8rem] font-semibold hover:border-border-2">Ajouter</button>
          </div>
        </CarteForm>
      </>
    );
    ...
    if (step === 3) return (
      <>
        <CarteForm titre="Localisation & itinéraire" icon={MapPin}>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Lieu de départ"><input className={inputCls} value={d.lieuDepart} onChange={(e) => set("lieuDepart", e.target.value.slice(0, 120))} placeholder="Valentine" /></Field>
            <Field label="Destination"><input className={inputCls} value={d.destination} onChange={(e) => set("destination", e.target.value.slice(0, 120))} placeholder="Saint-Denis" /></Field>
          </div>
          <Field label="Aperçu de l'itinéraire (carte à l'ancienne)"><CarteAncienne depart={d.lieuDepart} arrivee={d.destination} /></Field>
          <Field label="Description de l'itinéraire" hint="Points de passage, checkpoints, embuscades connues, route de secours…"><textarea className={inputCls + " min-h-[70px] resize-y"} value={d.itineraire} onChange={(e) => set("itineraire", e.target.value.slice(0, 600))} placeholder="Passer par le pont de Flatneck, éviter la route principale…" /></Field>
          <div className="grid gap-3 sm:grid-cols-3">
            <Field label="Distance"><input className={inputCls} value={d.distance} onChange={(e) => set("distance", e.target.value.slice(0, 40))} placeholder="~40 km" /></Field>
        ...
        <CarteForm titre="Planification" icon={CalendarClock}>
          <Field label="Phase de départ"><Puces options={PHASES} value={d.phase} onChange={(v) => set("phase", v)} /></Field>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Date de début"><input className={inputCls} type="date" value={d.dateDebut} onChange={(e) => set("dateDebut", e.target.value)} /></Field>
          ...
          <Field label="Notes logistiques"><textarea className={inputCls + " min-h-[70px] resize-y"} value={d.notesLogistiques} onChange={(e) => set("notesLogistiques", e.target.value.slice(0, 500))} placeholder="Ravitaillement en route, points de secours…" /><Compteur v={d.notesLogistiques} max={500} /></Field>
        </CarteForm>
      </>
    );
    ...
    if (step === 4) return (
      <CarteForm titre="Rémunération & partage" icon={Wallet}>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Bonus / récompense exceptionnelle"><input className={inputCls} value={d.bonus} onChange={(e) => set("bonus", e.target.value.slice(0, 120))} placeholder="Cheval de race si aucune perte" /></Field>
        ...
        <div className="rounded-lg border border-border bg-surface-2 px-3 py-2 text-[0.78rem] text-faint">Le paiement réel s'effectue à la clôture de l'opération (crédit du coffre du pôle). Ces montants figurent au dossier comme engagement.</div>
      </CarteForm>
    );
