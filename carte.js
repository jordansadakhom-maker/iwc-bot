...
async function httpHandle(req, res, client) {
  ...
  try {
    ...
    if (u.pathname === '/carte/data') {
      ...
      res.end(JSON.stringify({ level, types: TYPES.map(t => ({ key: t.key, label: t.label, emoji: t.emoji })), routeTypes: ROUTE_TYPES.map(t => ({ key: t.key, label: t.label, emoji: t.emoji, color: t.color })), niveaux: _niveauxAutorises(level), points: pts, routes, kmLargeur: (loadDB().carte?.kmLargeur || null) })); return true;
    }
...
var ROUTES=[],RTYPES=[],RHIDDEN={},MODE='pts',DRAW=[],KM=null;
...
function renderRoutes(){var svg=document.getElementById('routes');if(!svg)return;sizeSvg();var q=(SEARCH||'').toLowerCase();var h='';for(var i=0;i<ROUTES.length;i++){var r=ROUTES[i];if(RHIDDEN[r.type])continue;if(!r.points||r.points.length<2)continue;if(q&&(r.nom||'').toLowerCase().indexOf(q)<0&&(r.notes||'').toLowerCase().indexOf(q)<0)continue;var dd=smoothD(r.points);h+='<path class="rhalo" d="'+dd+'" vector-effect="non-scaling-stroke"></path><path class="rline" data-rid="'+r.id+'" d="'+dd+'" stroke="'+rcolor(r.type)+'" vector-effect="non-scaling-stroke"></path>';}if(MODE==='route'&&DRAW.length){var dp=DRAW.map(function(p){return p.x+','+p.y;}).join(' ');if(DRAW.length>1)h+='<polyline class="rline" points="'+dp+'" stroke="#f0c850" stroke-dasharray="2 1.5" vector-effect="non-scaling-stroke"></polyline>';for(var k=0;k<DRAW.length;k++)h+='<circle cx="'+DRAW[k].x+'" cy="'+DRAW[k].y+'" r="0.7" fill="#f0c850" stroke="#1a1208" stroke-width="0.25"></circle>';}svg.innerHTML=h;Array.prototype.forEach.call(svg.querySelectorAll('.rline[data-rid]'),function(el){el.addEventListener('click',function(e){e.stopPropagation();var r=ROUTES.find(function(x){return x.id===el.getAttribute('data-rid');});if(r)showRoute(r);});});}
