...
async function httpHandle(req, res, client) {
  ...
  try {
    ...
    if (u.pathname === '/carte/data') {
      ...
      res.end(JSON.stringify({ level, types: TYPES.map(t => ({ key: t.key, label: t.label, emoji: t.emoji })), routeTypes: ROUTE_TYPES.map(t => ({ key: t.key, label: t.label, emoji: t.emoji, color: t.color, dash: t.dash || null })), niveaux: _niveauxAutorises(level), points: pts, routes, kmLargeur: (loadDB().carte?.kmLargeur || null), calib: (loadDB().carte?.calib || null) })); return true;
    }
    ...
    // 🎯 Calibration coordonnées in-game → carte (2 repères : {mx,my,gx,gy})
    if (u.pathname === '/carte/calib' && req.method === 'POST') {
      if (level !== 'confidentiel') { res.writeHead(403); res.end('{}'); return true; }
      let body = ''; req.on('data', d => body += d); req.on('end', () => {
        try {
          const d = JSON.parse(body || '{}');
          if (d.clear) { const db = loadDB(); const c = _ensure(db); c.calib = null; saveDB(db); res.writeHead(200, { 'Content-Type': 'application/json' }); res.end('{"ok":true}'); return; }
          const num = v => Number(v);
          const okPt = p => p && ['mx', 'my', 'gx', 'gy'].every(k => isFinite(num(p[k])));
          if (!okPt(d.p1) || !okPt(d.p2)) { res.writeHead(400); res.end('{"error":"points"}'); return; }
          if (num(d.p1.gx) === num(d.p2.gx) || num(d.p1.gy) === num(d.p2.gy)) { res.writeHead(400); res.end('{"error":"aligned"}'); return; }
          const clean = p => ({ mx: Math.max(0, Math.min(100, num(p.mx))), my: Math.max(0, Math.min(100, num(p.my))), gx: num(p.gx), gy: num(p.gy) });
          const db = loadDB(); const c = _ensure(db); c.calib = { p1: clean(d.p1), p2: clean(d.p2), at: new Date().toISOString() }; saveDB(db);
...
#coordbar{position:fixed;top:58px;left:50%;transform:translateX(-50%);z-index:35;background:#241a12f2;border:1px solid #d9a441;border-radius:10px;padding:8px 10px;display:none;gap:8px;align-items:center;box-shadow:0 6px 20px #000a;max-width:94vw;flex-wrap:wrap}
#coordbar.on{display:flex}#coordbar span{font-size:13px;color:#f0c850}
#coordbar input{background:#15110c;color:#e8d8c0;border:1px solid #5a4632;border-radius:6px;padding:6px 8px;font-family:monospace;font-size:13px;width:140px}
#coordbar button{border:0;border-radius:6px;padding:7px 10px;cursor:pointer;font-weight:bold;font-family:inherit;font-size:13px}
#coordmark{position:absolute;transform:translate(-50%,-100%) scale(var(--ps,1));transform-origin:center bottom;z-index:7;pointer-events:none;font-size:24px;line-height:1;filter:drop-shadow(0 0 3px #000);display:none}
</style></head><body>
<header><b>🗺️ Carte — La Confrérie</b><span class="lvl">Accès : ${level === 'confidentiel' ? '🔴 Confidentiel' : level === 'membre' ? '🟡 Membre' : '🟢 Public'}</span><input id="search" placeholder="🔎 Chercher…"><select id="regionf"><option value="">📍 Toutes régions</option></select><button id="modebtn" class="modebtn">🧵 Tracer un itinéraire</button><button id="navbtn" class="modebtn">🧭 Trajet</button><button id="coordbtn" class="modebtn">📍 Coord.</button>${level === 'confidentiel' ? '<button id="scalebtn" class="modebtn">📏 Échelle</button>' : ''}<span class="hint">🖱️ Clic = point · 🧵 = itinéraire · 🧭 = trajet · 📍 = coords in-game</span></header>
<div id="stage"><div id="wrap"><img id="map" src="/carte/image?k=${tok}" alt="carte"><svg id="routes" viewBox="0 0 100 100" preserveAspectRatio="none"></svg><div id="coordmark">📍</div></div></div><div id="legend"></div><div id="coords">X — · Y —</div><div id="zoom"><button id="zin" title="Zoom +">＋</button><button id="zout" title="Zoom −">－</button><button id="zres" title="Réinitialiser">⟲</button></div><div id="drawbar"><span id="drawinfo">Itinéraire : 0 point</span><button id="drawundo" class="dbu">↶ Annuler</button><button id="drawfin" class="dbf">✓ Terminer</button><button id="drawcancel" class="dbc">✕</button></div><div id="navbar"><span id="navinfo">🧭 Clique le point de DÉPART</span><button id="naverase" class="dbc">✕ Effacer</button></div><div id="coordbar"></div><div id="mask"></div>
<script>
...
var CALIB=null,CALIBSTEP=0,CALIBREFS=[],COORDPT=null;
...
function setMode(m){MODE=m;var b=document.getElementById('modebtn'),nb=document.getElementById('navbtn'),cb=document.getElementById('coordbtn'),bar=document.getElementById('drawbar'),nbar=document.getElementById('navbar');if(b){b.classList.toggle('on',m==='route');b.textContent=m==='route'?'✋ Arrêter le tracé':'🧵 Tracer un itinéraire';}if(nb)nb.classList.toggle('on',m==='nav');if(cb)cb.classList.toggle('on',m==='coord');if(bar)bar.classList.toggle('on',m==='route');if(nbar)nbar.classList.toggle('on',m==='nav');if(m!=='route')DRAW=[];if(m!=='nav'){NAVA=null;NAVB=null;NAVPATH=null;NAVINFO='';}if(m!=='coord'){var mk=document.getElementById('coordmark');if(mk)mk.style.display='none';COORDPT=null;CALIBSTEP=0;CALIBREFS=[];}updateDrawBar();updateNavBar();updateCoordBar();renderRoutes();renderDrawHandles();}
function load(){fetch('/carte/data?k='+TOK).then(function(r){return r.json();}).then(function(j){if(j.error){document.body.innerHTML='<h2 style=text-align:center;margin-top:60px>Lien expiré</h2>';return;}DATA=j;ROUTES=j.routes||[];RTYPES=j.routeTypes||[];KM=j.kmLargeur||null;CALIB=j.calib||null;render();updateCoordBar();});}
...
// ── Coordonnées in-game ↔ carte (calibration linéaire à 2 repères) ──
function _calibXform(){if(!CALIB||!CALIB.p1||!CALIB.p2)return null;var a=CALIB.p1,b=CALIB.p2;if(a.gx===b.gx||a.gy===b.gy)return null;var ax=(b.mx-a.mx)/(b.gx-a.gx),bx=a.mx-ax*a.gx,ay=(b.my-a.my)/(b.gy-a.gy),by=a.my-ay*a.gy;return {ax:ax,bx:bx,ay:ay,by:by};}
function gameToMap(gx,gy){var t=_calibXform();if(!t)return null;return {x:t.ax*gx+t.bx,y:t.ay*gy+t.by};}
function mapToGame(mx,my){var t=_calibXform();if(!t||t.ax===0||t.ay===0)return null;return {gx:(mx-t.bx)/t.ax,gy:(my-t.by)/t.ay};}
function _parseCoords(s){return (String(s).match(/-?\d+(?:[.,]\d+)?/g)||[]).map(function(v){return parseFloat(v.replace(',','.'));}).filter(function(n){return isFinite(n);});}
function centerOn(px,py){var st=document.getElementById('stage');if(!st)return;var r=st.getBoundingClientRect();var iw=mapImg.clientWidth||r.width,ih=mapImg.clientHeight||r.height;if(SC<4)SC=4;OX=r.width/2-(px/100)*iw*SC;OY=r.height/2-(py/100)*ih*SC;wrap.style.transform='translate('+OX+'px,'+OY+'px) scale('+SC+')';wrap.style.setProperty('--ps',1/SC);}
function showCoords(gx,gy){var m=gameToMap(gx,gy);var mk=document.getElementById('coordmark');if(!m){if(mk)mk.style.display='none';COORDPT=null;updateCoordBar();return;}COORDPT={x:Math.max(0,Math.min(100,m.x)),y:Math.max(0,Math.min(100,m.y)),gx:gx,gy:gy};if(mk){mk.style.left=COORDPT.x+'%';mk.style.top=COORDPT.y+'%';mk.style.display='block';}centerOn(COORDPT.x,COORDPT.y);updateCoordBar();}
function coordMapClick(x,y){
  if(CALIBSTEP>0){var gxs=prompt('Coordonnée in-game X de ce lieu ?','');if(gxs===null)return;var gys=prompt('Coordonnée in-game Y de ce lieu ?','');if(gys===null)return;var gx=parseFloat(String(gxs).replace(',','.')),gy=parseFloat(String(gys).replace(',','.'));if(!isFinite(gx)||!isFinite(gy)){alert('Coordonnées invalides.');return;}CALIBREFS[CALIBSTEP-1]={mx:+x.toFixed(2),my:+y.toFixed(2),gx:gx,gy:gy};if(CALIBSTEP<2){CALIBSTEP=2;updateCoordBar();}else{if(CALIBREFS[0].gx===CALIBREFS[1].gx||CALIBREFS[0].gy===CALIBREFS[1].gy){alert('Les deux lieux doivent avoir des X ET des Y différents. Reclique le 2e lieu.');return;}fetch('/carte/calib?k='+TOK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({p1:CALIBREFS[0],p2:CALIBREFS[1]})}).then(function(r){return r.json();}).then(function(j){CALIBSTEP=0;CALIBREFS=[];if(j&&j.error){alert('Calibration refusée ('+j.error+').');}load();});}return;}
  var g=mapToGame(x,y);if(g){COORDPT={x:+x.toFixed(2),y:+y.toFixed(2),gx:Math.round(g.gx*100)/100,gy:Math.round(g.gy*100)/100};var mk=document.getElementById('coordmark');if(mk){mk.style.left=x+'%';mk.style.top=y+'%';mk.style.display='block';}updateCoordBar();}
}
function updateCoordBar(){var bar=document.getElementById('coordbar');if(!bar)return;if(MODE!=='coord'){bar.classList.remove('on');return;}var t=_calibXform();var h='';
  if(CALIBSTEP>0){h='<span>🎯 Calibration '+CALIBSTEP+'/2 — clique un LIEU CONNU sur la carte, puis entre ses coords in-game.</span><button id="cbcancel" class="dbc">✕ Annuler</button>';}
  else if(!t){h='<span>📍 Carte pas encore calibrée.</span>'+(LVL==='confidentiel'?'<button id="cbstart" class="dbf">🎯 Calibrer (2 repères)</button>':'<span>Demande à la Direction de calibrer.</span>')+'<button id="cbclose" class="dbc">✕</button>';}
  else{h='<span>📍</span><input id="coordin" placeholder="X, Y in-game" value="'+(COORDPT?(COORDPT.gx+', '+COORDPT.gy):'')+'"><button id="cbgo" class="dbf">Aller</button>';if(COORDPT)h+='<span>→ carte X '+COORDPT.x.toFixed(1)+' · Y '+COORDPT.y.toFixed(1)+'</span><button id="cbplace" class="dbu">📌 Placer ici</button>';h+=(LVL==='confidentiel'?'<button id="cbrecal" class="dbu">↻ Recalibrer</button>':'')+'<button id="cbclose" class="dbc">✕</button>';}
  bar.innerHTML=h;bar.classList.add('on');
  var e;
  e=document.getElementById('cbcancel');if(e)e.onclick=function(){CALIBSTEP=0;CALIBREFS=[];updateCoordBar();};
  e=document.getElementById('cbstart');if(e)e.onclick=function(){CALIBSTEP=1;CALIBREFS=[];updateCoordBar();};
  e=document.getElementById('cbrecal');if(e)e.onclick=function(){CALIBSTEP=1;CALIBREFS=[];COORDPT=null;var mk=document.getElementById('coordmark');if(mk)mk.style.display='none';updateCoordBar();};
  e=document.getElementById('cbclose');if(e)e.onclick=function(){setMode('pts');};
  e=document.getElementById('cbgo');if(e)e.onclick=function(){var n=_parseCoords(document.getElementById('coordin').value);if(n.length<2){alert('Entre au moins X et Y.');return;}showCoords(n[0],n[1]);};
  e=document.getElementById('cbplace');if(e)e.onclick=function(){if(COORDPT)showAdd(COORDPT.x,COORDPT.y);};
  var ci=document.getElementById('coordin');if(ci)ci.addEventListener('keydown',function(ev){if(ev.key==='Enter'){var n=_parseCoords(ci.value);if(n.length>=2)showCoords(n[0],n[1]);}});
}
...
var _cb=document.getElementById('coordbtn');if(_cb)_cb.onclick=function(){setMode(MODE==='coord'?'pts':'coord');};
