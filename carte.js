...
function setMode(m){MODE=m;var b=document.getElementById('modebtn');var bar=document.getElementById('drawbar');if(b){b.classList.toggle('on',m==='route');b.textContent=m==='route'?'✋ Arrêter le tracé':'🧵 Tracer un itinéraire';}if(bar)bar.classList.toggle('on',m==='route');if(m!=='route')DRAW=[];updateDrawBar();renderRoutes();renderDrawHandles();}
